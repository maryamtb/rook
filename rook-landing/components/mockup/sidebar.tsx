...
export function Sidebar({ t, variant }: { t: ThemeColors; variant: MockupVariant; }) {
  return (
    <div
      className="w-[200px] shrink-0 hidden sm:flex sm:flex-col self-stretch transition-colors duration-500"
    ...
    >
      <div className="px-2 pt-2.5 pb-1.5">
        ...
        >
          ...
          <span className="text-[9px] font-mono transition-colors duration-500 shrink-0" style={{ color: `${t.subtext}cc` }}>
            {"⌘"}K
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 px-2 pt-1 space-y-0.5 overflow-hidden">
        ...
        >
          ...
          <span className="text-xs transition-colors duration-500 flex-1 truncate" style={{ color: t.text }}>
        ...
        <div className="flex items-center gap-1.5 px-1.5 py-[5px] rounded-md mt-0.5">
          <svg className="w-[11px] h-[11px] transition-colors duration-500 shrink-0" style={{ color: `${t.subtext}cc` }} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 16 16">
          ...
          <span className="text-xs transition-colors duration-500 flex-1 truncate" style={{ color: t.subtext }}>
            {variant === "dsa" ? "System Design" : "Work Notes"}
          </span>
          <span
            className="text-[9px] px-1.5 py-0.5 rounded-full transition-colors duration-500 shrink-0"
            style={{ color: `${t.subtext}cc`, backgroundColor: t.surface }}
          >
            2
          </span>
        </div>
...
function NoteItem({ label, active, t }: { label: string; active?: boolean; t: ThemeColors; }) {
  return (
    ...
    >
      <svg className="w-[10px] h-[10px] transition-colors duration-500 shrink-0" style={{ color: active ? t.accent : `${t.subtext}cc` }} viewBox="0 0 16 16">
        {active ? (
